...
class User:
    ...
    def clear():
        ...
        connectToMySQL(User.DB).query_db(query)
        query= "DELETE FROM likes WHERE user_id IS NOT null;"
        connectToMySQL(User.DB).query_db(query)
    ...
    @classmethod
    def like_pokemon(cls, data):
        query = """INSERT INTO likes (user_id, pokemon_id)
                    VALUES (%(user_id)s, %(pokemon_id)s);"""
        result = connectToMySQL(User.DB).query_db(query, data)
        return result
    

    @classmethod
    def get_user_liked_pokemon( cls , id ):
        query = "SELECT * FROM users LEFT JOIN likes ON likes.user_id = users.id LEFT JOIN pokemons ON likes.pokemon_id = pokemons.id WHERE users.id = %(id)s;"
        results = connectToMySQL(User.DB).query_db( query , {"id":id} )
        print(results)
        return results